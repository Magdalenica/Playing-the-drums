...
window.addEventListener('keydown',(event)=>{
   let code = event.keyCode;
    ...
    if(!keyElement){
       return;// ako ne postoji, treba da prekinemo izvrsavanje ovog listenera, tj. funkcije
    }else{
        console.log(keyElement)
    }
    
   let audio = document.querySelector(`audio[data-key="${code}"]`)
   audio.currentTime = 0;// da vratimo zvuk na nulu, jer u suprotnom culi bismo svaki drugi zvuk;
   audio.play();
   console.log(audio)
